    ...
    tabItem(tabName = "introduction",
            ...
            fluidRow(
              box(width = 12, background = "yellow",
                  h4("Denne platform er designet som et redskab til computerbaseret tekstanalyse. Alle funktioner er baseret på Natural Language Processing (NLP) eller som det på dansk hedder Sprogteknologi. Denne tværfaglige disciplin handler om at få computere til at forstå sprog med det formål at trække meningsfuld information ud af store mængder tekst. I en digitaliseret verden, hvor store mængder tekst er frit tilgængeligt kan værktøjer, som denne platform tilbyder, muliggøre tekstanalyse med en kvantitativ tilgang fremfor det traditionelle, kvalitative arbejde med tekster, som vi er vant til. På denne platform får du mulighed for at arbejde med forskellige NLP-værktøjer: Topic Modeling, Sentiment Analysis, Word Clouds, og en semantisk model trænet på den hidtil største samling af dansk sprogbrug i alle former.")
                )
            ),
  ...
  ### ---------- DAGW MODEL ---------- ###
    tabItem(tabName = "semantics",
      ...
      # SEMANTIC SIMILARITY #       
            ...
            fluidRow(
              box(title = span(strong("Find den semantiske lighed mellem to ord"),
                               ...
                               bsPopover(id = "info_nocapitalletters", title = "",
                                         ...
                                         placement = "right",
                                         options = list(container = "body")
                               ), style = "color:orange"), width = 6, height = 250,
    ...
    # GITHUB LINK #
            fluidRow(
              box(width = 12, background = "yellow",
                         ...
                         h4("Modellen er tilgængelig på vores GitHub: https://github.com/TheNLPlayPlatform/NLPlay"), align = "center"
                    )
            )
    ),
...
### ---------- THE DAGW MODEL ---------- ###
  ...
  # react_dissimilar_words <- reactiveValues(dissimilar_words = NULL)

    # Table Output: Similar words
  output$table_similar <- renderTable({
    
    #if (input$similar_dissimilar == "similar"){
    ...
    # if (input$similar_dissimilar == "dissimilar"){
    #   dissimilar_words <- calc_dissimilar_words(word = req(input$sm_text1), embedding_matrix = matrix_DAGW_model, n = 5)
    #   dissimilar_words <- as.list(dissimilar_words)
    #   dissimilar_words <- as.data.frame(dissimilar_words)
    #   react_dissimilar_words$dissimilar_words <- dissimilar_words
...
### ---------- TOPIC MODELING ---------- ### 
  ...
  tm_manual <- reactive({
    ...
    tm_data4 <- data.frame(title = input$tm_title4, text = input$tmtext4)
    tm_data5 <- data.frame(title = input$tm_title5, text = input$tmtext5)
    ...
    if(input$stem == TRUE) {
      ...
      tm_data4$text <- tm::stemDocument(as.character(tm_data4$text),"danish")
      tm_data5$text <- tm::stemDocument(as.character(tm_data5$text),"danish")
    }
    
    tm_data <- rbind(tm_data1,tm_data2, tm_data3, tm_data4, tm_data5)
    output$upload_complete <- renderText({ "Din fil er nu uploadet!" })
  })
  ...
  output$gamma_plot <- renderPlotly({
    ...
    if (input$tm_plot_source == "rb_tops") {
      ggplot_gamma_tekster <- ggplot(td_gamma, aes(gamma, fill = as.factor(topic))) + 
...
### ---------- EXERCISES ---------- ###
  ...
  output$downloadData_corpus <- downloadHandler(
    ...
    content = function(file) {
      write.csv(file1, file, row.names = FALSE)
    }
  )
  ...
  output$downloadData_1999 <- downloadHandler(
    ...
    content = function(file) {
      #file2 <- paste0('/NLPlay/www/', "1999.txt", collapse = NULL)
      #file.copy(file2, file)
      write_delim(file2, file, col_names = FALSE)
    }
  )
